                ...
                <div class="row">
                    <div class="col-xl-8">
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group mb-2 mb-lg-3">
                                            <label for="unit" class="col-from-label fs-14 fw-500">{{ translate('Unit') }}
                                                ...
                                                <input type="text" letter-only class="form-control @error('unit') is-invalid @enderror" name="unit" value="{{ $product->getTranslation('unit', $lang) }}" placeholder="{{ translate('KG, Pc') }}">
                                        </div>
                                    </div>
                        ...
                        <div class="border border-gray-300 rounded-2 px-3 px-lg-4 py-3 py-lg-4 mt-4">
                            ...
                            <div id="show-hide-div" @if ($product->variant_product == 1) style="display:none;" @endif>
                                <div class="row gutters-5 mt-3">
                                    <!-- Stock -->
                        ...
                        @php
                            // Parse existing stocks into editor rows. Variant strings are
                            // "<ColorName>-<Val1>-<Val2>" with spaces stripped from each value.
                            // The '-' separator is also legal inside a value (e.g. "Medium - Regular"
                            // -> "Medium-Regular"), so we greedily match against the known option
                            // values per attribute instead of a naive explode('-').
                            $rs_choice_options_arr = json_decode($product->choice_options ?? '[]') ?? [];
                            $rs_color_codes = json_decode($product->colors ?? '[]', true) ?? [];
                            $rs_all_colors = \App\Models\Color::orderBy('name')->pluck('name', 'code')->toArray();

                            // Per-attribute candidate values (space-stripped => raw), longest first
                            // so greedy prefix matching prefers "Medium-Regular" over "Medium".
                            $rs_attr_values = [];
                            foreach ($rs_choice_options_arr as $rs_co) {
                                $rs_vals_map = [];
                                foreach (($rs_co->values ?? []) as $rs_v) {
                                    $rs_vals_map[str_replace(' ', '', $rs_v)] = $rs_v;
                                }
                                uksort($rs_vals_map, fn($a, $b) => strlen($b) <=> strlen($a));
                                $rs_attr_values[$rs_co->attribute_id] = $rs_vals_map;
                            }

                            $rs_initial_rows = [];
                            if ($product->variant_product == 1) {
                                foreach ($product->stocks as $rs_stock) {
                                    if ($rs_stock->variant === '' || $rs_stock->variant === null) {
                                        continue;
                                    }
                                    $rs_rest = $rs_stock->variant;
                                    $rs_color_code = '';
                                    foreach ($rs_color_codes as $rs_code) {
                                        $rs_cname = $rs_all_colors[$rs_code] ?? null;
                                        if ($rs_cname !== null && ($rs_rest === $rs_cname || str_starts_with($rs_rest, $rs_cname . '-'))) {
                                            $rs_color_code = $rs_code;
                                            $rs_rest = ltrim(substr($rs_rest, strlen($rs_cname)), '-');
                                            break;
                                        }
                                    }
                                    ...
                                    $rs_opts = [];
                                    foreach ($rs_choice_options_arr as $rs_co) {
                                        $rs_aid = $rs_co->attribute_id;
                                        $rs_matched = '';
                                        // try the known values (longest first) as a prefix of the remainder
                                        foreach (($rs_attr_values[$rs_aid] ?? []) as $rs_stripped => $rs_raw) {
                                            if ($rs_rest === $rs_stripped || str_starts_with($rs_rest, $rs_stripped . '-')) {
                                                $rs_matched = $rs_raw;
                                                $rs_rest = ltrim(substr($rs_rest, strlen($rs_stripped)), '-');
                                                break;
                                            }
                                        }
                                        if ($rs_matched === '') {
                                            // fallback: consume up to next '-'
                                            $rs_pos = strpos($rs_rest, '-');
                                            if ($rs_pos === false) { $rs_matched = $rs_rest; $rs_rest = ''; }
                                            else { $rs_matched = substr($rs_rest, 0, $rs_pos); $rs_rest = substr($rs_rest, $rs_pos + 1); }
                                        }
                                        $rs_opts[$rs_aid] = $rs_matched;
                                    }

                                    $rs_initial_rows[] = [
                                        'color' => $rs_color_code,
                                        'opts'  => $rs_opts,
                                        'price' => $rs_stock->price,
                                        'sku'   => $rs_stock->sku,
                                        'qty'   => $rs_stock->qty,
                                        'img'   => $rs_stock->image,
                                    ];
                        ...
                        @endphp
                        @if (feature_allowed('product_variations'))
                        <div class="border border-gray-300 rounded-2 px-3 px-lg-4 py-3 py-lg-4 mt-4 mb-4 mb-xl-0" id="variant-div-show-hide" @if ($product->variant_product != 1) style="display:none;" @endif>
...
<script type="text/javascript">
    ...
    var rsAllColorNames = @json($rs_all_colors); // code => name
    var rsVariantRows = @json($rs_initial_rows);
    var rsRowSeq = 0;
    ...
    // block submit while a variation row is missing a dropdown choice
