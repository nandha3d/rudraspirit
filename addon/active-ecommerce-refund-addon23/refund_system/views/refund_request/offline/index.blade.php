    ...
    <div class="col-12 col-sm-12 col-lg-10 mx-auto">
        <div class="aiz-titlebar text-left pb-5px">
            <div class="row align-items-center">
                <div class="col-auto">
                    <h1 class="h3 fw-bold">{{ translate('All Offline Payout Methods') }}</h1>
                </div>
            </div>
        </div>
        <div class="card">
            ...
            <div class="d-flex align-items-center justify-content-between flex-wrap border-bottom  border-light px-25px table-nav-tabs pb-3 pb-xl-0">
                <div class="table-tabs-container flex-grow-1">
                    <ul class="nav nav-tabs border-0 " id="myTab" role="tablist">
                        @foreach ($payout_method_tabs as $payout_method_tab)
                        <li class="nav-item" role="presentation">
                            <button class="nav-link px-0 pb-15px fs-14 fw-500 {{ $loop->first ? 'active' : '' }}" data-toggle="tab"  role="tab" aria-selected="{{ $loop->first ? 'true' : 'false' }}"
                                id="{{ Str::slug($payout_method_tab) }}-tab"  onclick="changeTab(this, '{{ Str::slug($payout_method_tab) }}')" role="tab" aria-controls="{{ Str::slug($payout_method_tab) }}">
                                {{ translate($payout_method_tab) }}
                            </button>
                        </li>
                        @endforeach
                    </ul>
                ...
                <div class="mb-3 mb-md-0">
                    @can('add_payout_method_for_customer')
                     <a href="{{ route('payout_method_for_customer_create') }}" class="position-relative overflow-hidden add-new-btn">
                        <span class="position-relative z-2 pr-15px fs-14 fw-500 text-blue label-text">{{ translate('Add New') }}</span>
            ...
            <div class="tab-filter-bar">
                <form class="" id="sort_payout_methods" action="" method="GET">
                        ...
                        <div class="col px-0">
                            <div class="input-group mb-0 border border-light px-3 bg-light rounded-1">
                                ...
                                <input type="text" class="form-control form-control-sm border-0 px-2 bg-transparent"
                                    id="search_input" name="search"placeholder="{{translate('Search Payout Method ...')}}">
                            </div>
                        </div>
...
<script type="text/javascript">
    let currentTab = '{{ Str::slug($payout_method_tabs[0]) }}';
    var searchTimer;
    ...
    function sort_payout_methods(el){
        $('#sort_payout_methods').submit();
    }
    function single_delete(refundMethodId) {
        if('{{env('DEMO_MODE')}}' == 'On'){
        ...
        $.ajax({
            url: "{{ route('payout_method_for_customer_delete', ':id') }}".replace(':id', refundMethodId),
            type: 'GET',
            success: function(response) {
                if (response == 1) {
                    AIZ.plugins.notify('success', '{{ translate('Selected item deleted successfully') }}');
                    hideBulkActionModal();
                    getRefundMethods(currentTab);
                }
            }
        });
    }
    ...
    function bulk_delete() {
        ...
        var data = new FormData($('#sort_payout_methods')[0]);
        $.ajax({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            url: "{{route('payout_method_for_customer_bulk_delete')}}",
            type: 'POST',
            data: data,
            cache: false,
            contentType: false,
            ...
            success: function (response) {
                if(response == 1) {
                    AIZ.plugins.notify('success', 'Selected Offline Payout Mehtods Deleted successfully');
                    hideBulkActionModal(); 
                    getRefundMethods(currentTab);
                }
            },
    ...
    function bulkDeleted() {
        if ($('.check-one:checked').length == 0) {
            AIZ.plugins.notify('danger', '{{ translate('Please select at least one offline payout method') }}');
            return;
        }
        showBulkActionModal();
        $('#confirmation-title').text('{{ translate('Delete Confirmation') }}');
        $('#confirmation-question').text('{{ translate('Are you sure you want to delete the selected offline payout methods?') }}');
    ...
    function singleDelete(refundMethodId) {
            showBulkActionModal();
            $('#confirmation-title').text('{{ translate('Delete Confirmation') }}');
            $('#confirmation-question').text('{{ translate('Are you sure you want to delete the selected offline payout method?') }}');
            $('#conform-yes-btn').attr("onclick", "single_delete(" + refundMethodId + ")");
            $('.confirmation-icon').addClass('d-none');
            $('#delete-confirm-icon').removeClass('d-none');
           
        }
    
    function getRefundMethods(slug, page = 1) {
        var type = $('#type').val();
        ...
        $.ajax({
            url: `{{ route('payout_method_for_customer_filter' ) }}?page=${page}`,
            method: 'GET',
            data: { type: type, payout_method_for_customer_type: slug, search: keyword },
            success: function(response) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
     ...
     $('#search_input').on('keyup', function () {
        ...
        $(document).on('click', '.pagination a', function(e) {
            ...
            getRefundMethods(currentTab, page);
        });
