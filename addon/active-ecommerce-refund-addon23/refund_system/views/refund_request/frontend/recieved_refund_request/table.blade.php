    ...
    <table class="table mb-0" id="aiz-data-table">
        ...
        <tbody>
            ...
            <tr class="data-row">
                ...
                <td class="align-middle" data-label="Refund Code">
                ...
                <td class="hide-6xl align-middle" data-label="Dispute Status">
                    <div class="w-150px w-md-150px">
                        <div class="pb-1">
                            <span
                                class="text-dark fs-12 fw-400">
                                @if ($refund_request->dispute_refund_status == 2)
                                    <span class="badge badge-inline badge-success fs-11">{{translate('Approved')}}</span>
                                @elseif ($refund_request->dispute_refund_status == 3)
                                    <span class="badge badge-inline badge-danger fs-11">{{translate('Rejected')}}</span>
                                @elseif ($refund_request->dispute_refund_status == 1)
                                    <span class="badge badge-inline badge-info fs-11">{{translate('Pending')}}</span>
                                @else
                                    <span class="badge badge-inline badge-info fs-11">{{translate('N/A')}}</span>
                                @endif
                            </span>
                        </div> 
                    </div>
                    ...
                    <div class="d-flex align-items-center justify-content-end">
                        <div class="dropdown float-right">
                            ...
                            <div class="dropdown-menu dropdown-menu-right dropdown-menu-sm">
                                ...
                                <a href="{{ route('seller.orders.show', encrypt($refund_request->order->id)) }}" title="{{ translate('View Order') }}"
                                    class="d-flex align-items-center px-20px py-10px hov-bg-light hov-text-blue text-dark">
                                    <span
                                        class="fs-12 fw-500 pl-10px">{{ translate('View Order') }}</span>
                                </a>
                                @if ($refund_request->preferred_payment_channel == 'offline' && get_setting('product_manage_by_admin') == 0 && get_setting('seller_product_refund_approval') == 'seller_can_refund_directly')
                                    <a href="#" id="view_payment_info" data-user-id="{{ $refund_request->id }}" title="{{ translate('View Payment Info') }}"
                                        class="d-flex align-items-center px-20px py-10px hov-bg-light hov-text-blue text-dark">
                                        <span
                                            class="fs-12 fw-500 pl-10px">{{ translate('View Payment Info') }}</span>
                                    </a>
                                @endif
                            </div>
                        </div>
