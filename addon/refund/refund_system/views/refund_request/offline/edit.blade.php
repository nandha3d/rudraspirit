        ...
        <div class="col-lg-8 mx-auto">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6">{{ translate('Edit Offline Payout Method Information') }}</h5>
                </div>
                <div class="card-body">
                    <form class="form-horizontal"
                        action="{{ route('payout_method_for_customer_update', $payout_method->id) }}" method="POST"
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="type" class="form-control aiz-selectpicker" required>
                                ...
                                <option value="bank_payment" {{ (isset($payout_method) && $payout_method->type == 'bank_payment') ? 'selected' : '' }}>
                                    {{ ucfirst(translate('Bank Payment')) }}
                                </option>
                                <option value="others" {{ (isset($payout_method) && $payout_method->type == 'others') ? 'selected' : '' }}>
                                    {{ ucfirst(translate('Others')) }}
                                </option>
                            </select>
                        </div>
